...
不要通过访问 B 站动态网页来截图，也不要把 B 站网页 DOM 作为业务解析接口。
...
```text
新闻列表页 → 文章详情页 → 内容解析 → 本地图片模板
```

官网图片推送时优先直接打开文章详情页，截取 `#NIE-art` 容器以保留官网原始样式；如果页面不可访问，再使用本地 HTML 模板截图。两条路径都必须经过 Playwright，不得恢复 Pillow 业务渲染分支。
...
- B站：本地 HTML 模板渲染，参考 B站动态卡片视觉，再由 Playwright 截图。
- 官网：优先截取 `#NIE-art` 正文容器，失败后使用本地 HTML 模板；两者都由 Playwright 截图。
- 内容过长时分页，不能静默截断。
- 图片底部应保留原文链接或来源信息。
- 图片发送失败时回退 Markdown，再回退纯文本。
- 图片资源应设置超时、缓存和失败占位处理。
...
后续开发优先补充真实接口联调、官网选择器适配、长图分页、图片资源缓存、推送记录管理和部署配置；不得跳过凭据安全、采集游标、去重和推送队列。
