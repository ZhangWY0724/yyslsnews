...
B站动态发现和去重必须使用结构化接口。普通动态截图详情页正文；视频动态使用本地 HTML 模板，根据结构化数据生成视频投稿卡片，不打开视频播放详情页：

```text
bilibili-api-python
    → 结构化动态数据
    → 本地解析模型
    → 普通动态：详情页正文截图 / 视频动态：标题、文案和封面本地模板
    → Playwright 截图生成图片
    → QQBot 图片消息
```

网页 DOM 仅用于定位普通动态的截图区域，不作为动态发现、正文解析或去重接口。视频卡片内容来自结构化动态数据。
图文等 Opus 动态优先使用结构化接口中的 `major.opus.jump_url` 打开详情页，避免旧动态域名展示登录页。
...
```text
新闻列表页 → 标题和详情 URL 入库 → 推送时打开详情页 → Playwright 截取 `.container-all.news`
```

官网图片推送直接打开文章详情页，截取 `.container-all.news` 容器以保留官网原始样式；`#NIE-art` 只用于检查正文图片加载。列表分页链接不得作为文章入库；页面或容器不可访问时发送标题和原文 URL 的纯文本消息。
...
同一内容的图片渲染产物由所有推送目标共享；待所有目标任务结束后清理，重试期间不得重复渲染。
...
- B站普通动态：截取动态详情页正文为一张长图；视频动态：使用结构化字段在本地模板中渲染标题、文案、封面和视频链接；渲染失败时发送标题和原文 URL 的纯文本消息。
- 官网：直接截取 `.container-all.news` 容器；超高正文在浏览器中按原宽分页；截图失败时发送标题和原文 URL 的纯文本消息。
- 截图统一使用 1920×1080 视口、1 倍设备像素，按网页 100% 比例截取指定区域。
- B站优先保持单张长图；超出 QQ 图片可接受高度时按原始宽度分页顺序发送，不能静默截断。
- 纯文本兜底消息必须附原文链接。
- 图片上传或发送失败时直接发送带原文 URL 的纯文本消息；纯文本发送失败仍由 Outbox 记录并重试。
- 图片资源应设置超时；共享截图在同一内容的目标任务全部结束后清理。
